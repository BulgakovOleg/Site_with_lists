...
@app.route("/")
def hello_world():
    return '<h1>Hello, World!</h1> <a href="/random_fact">Посмотреть случайный факт!</a> <a href = "/data">Особый факт!</a>'
@app.route("/random_fact")
def random_fact():
    return f'<p>{random.choice(facts_list)}</p>'
@app.route("/data")
def data():
    return f'<p>{"Здесь пока ничего нет,возможно здесь что-то появится"}</p>'
app.run(debug=True)